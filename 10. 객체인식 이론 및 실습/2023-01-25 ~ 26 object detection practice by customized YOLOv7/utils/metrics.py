...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes | 각 클래스별
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=1, label=f'{names[i]} {ap[i, 0]:.3f}')  # plot(recall --> px, precision --> py)

            # if names[i] == "dog" : # 21개 넘는 label에 대해선 하나씩 if문 걸어서 값 얻어내서 csv 저장하고 그래프 따로 그려줘야 함
            #     print(px, y)
    else:
        ax.plot(px, py, linewidth=1, color='grey')  # plot(recall, precision), 전체 class 평균 그래프
